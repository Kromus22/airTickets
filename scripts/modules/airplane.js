...
const createBlockSeat = (n, count, bookingSeat) => {
  ...
  const fuselage = createElem('ol', {
    className: 'fuselage',
  })

  for (let i = n; i < count + n; i++) {
    const wrapperRow = createElem('li')
    const seats = createElem('ol', {
      className: 'seats',
    })

    const seatsRow = letters.map(letter => {
      const seat = createElem('li', {
        className: 'seat',
      })

      const wrapperCheck = createElem('label')
...
const createAirplane = (bookingSeat, title, tourData) => {
  ...
  const choisesSeat = createElem('form', {
    className: 'choises-seat',
  })

  const plane = createElem('fieldset', {
    className: 'plane',
    name: 'plane',
  })
...
const checkSeat = (bookingSeat, form, data, id) => {
  ...
  form.addEventListener('submit', (e) => {
    ...
    document.body.innerHTML = `
      <h1 class="title">Спасибо, хорошего путешествия!</h1>
      <h2 class="title">${booking.length === 1 ?
        `Ваше место ${booking}` : `Ваши места ${booking}`}</h2>
    `
  })
}
